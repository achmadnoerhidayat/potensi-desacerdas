...
session_start();
	...
	$kode=$_GET['kd'];
	$thn=$_GET['thn'];
	$cari_kd=mysqli_query($koneksi,"SELECT anggaran, keterangan FROM tbrapbes_detail WHERE kode='$kode' and tahun='$thn' and kode_wil='$kd_kel'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$anggaran=$tm_cari['anggaran'];
	$ket=$tm_cari['keterangan'];
	
	$cari_kd=mysqli_query($koneksi,"SELECT nama FROM tbrek WHERE kode='$kode'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$nama_rek=$tm_cari['nama'];
	
	if(isset($_POST['btnSimpan'])) {
		$txttahun = $_POST['txtthn'];
		$txtkode = $_POST['txtkode'];
		$txtkdkel = $_POST['txtkdkel'];
		$txtanggaran = $_POST['txtanggaran'];	
		$txtket = $_POST['txtket'];
		
		$data = mysqli_query($koneksi,"SELECT * FROM tbrapbes_detail WHERE kode='$txtkode' and tahun='$txttahun' and kode_wil='$txtkdkel'"); 
		$cek = mysqli_num_rows($data);
		if($cek > 0){
			mysqli_query($koneksi,"UPDATE tbrapbes_detail SET anggaran='$txtanggaran', keterangan='$txtket' 
									WHERE kode='$txtkode' and tahun='$txttahun' and kode_wil='$txtkdkel'");
								
			$next = 'location:adm_rabdes_input_detail.php?thn=';
			$awal = $next.$txttahun;
			header($awal);
		} else {
			mysqli_query($koneksi,"INSERT INTO tbrapbes_detail (tahun, kode, uraian, anggaran, keterangan, kode_wil) 
								VALUES ('$txttahun','$txtkode','','$txtanggaran','$txtket', '$txtkdkel')");
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Home</a> 
					...
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="" class="breadcrumb--active">Input Rancangan APBDes</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
...
<div class="text-xs text-theme-41"><?php include "titel_status.php"; ?></div>
			...
			<input type="hidden" name="txtthn" value="<?php echo $thn; ?>"/>
			<input type="hidden" name="txtkode" value="<?php echo $kode; ?>"/>
                    ...
                    <div class="intro-y col-span-12 lg:col-span-12">
                        ...
                        <div class="intro-y box">
                            <div class="p-5" id="input">
                                <div class="preview">
									<div class="mt-3">
                                        <input type="text" class="input w-full border mt-2" id="txttahun1" name="txttahun1" 
										disabled value="<?php echo $thn; ?>" >
                                    
                                        <input type="text" class="input w-full border mt-2" id="txtkode1" name="txtkode1" 
										disabled value="<?php echo $kode; ?>" >

                                        <input type="text" class="input w-full border mt-2" id="txtnm1" name="txtnm1" 
										disabled value="<?php echo $nama_rek; ?>" >
                                    
                                        <input type="text" class="input w-full border mt-2" id="txtanggaran" name="txtanggaran" 
										required placeholder="Anggaran" value="<?php echo $anggaran; ?>" >
                                    
                                        <input type="text" class="input w-full border mt-2" id="txtket" name="txtket" 
										required placeholder="Keterangan" value="<?php echo $ket; ?>" >
                                    
                                    </div>	
									<div class="mt-3 center">
										<center>
											<button type="submit" name="btnSimpan" class="button bg-theme-1 text-white mt-5">   &nbsp;&nbsp;&nbsp;SIMPAN&nbsp;&nbsp;&nbsp;   </button>
										</center>
                                    </div>
								</div>
                            </div>
