...
session_start();
	...
	$cari_kd=mysqli_query($koneksi,"SELECT 
									kd_donasi, nm_donasi, penggalang, norek, nm_rek, bank, keterangan, kode, tgl_input, jam_input, user_id FROM tbdonasi 
									where kd_donasi='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
...
<a href="donasi_pemberi_rst_add.php?kd=<?php echo $id; ?>">Tambah Pemberi Donasi</a></button>
			</button>
                    </div>
                </div>
                    ...
                    <table class="table table-report table-report--bordered display datatable w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="border-b-2 text-right whitespace-no-wrap">Jumlah Donasi</th>
                                <th class="border-b-2 text-center whitespace-no-wrap">Tanggal</th>
                                <th class="border-b-2 whitespace-no-wrap">Bank</th>
                                <th class="border-b-2 whitespace-no-wrap">Rekening Atas Nama</th>
                                <th class="border-b-2 text-center whitespace-no-wrap">Mode Transfer</th>
                        ...
                        <tbody>
							...
							<?php 
								...
								$sql = mysqli_query($koneksi,"SELECT 
									nama, jumlah, DATE_FORMAT(tanggal,'%d/%m/%Y') AS tgl, bank, norek, nmrek, id, mode FROM tbdonasi_pemberi where kd_donasi='$id'");
								while ($tampil = mysqli_fetch_array($sql)) {
									$no++;
							?>
							
                            <tr>
                                <td class="border-b"><?php echo $tampil['nama']?></td>
                                <td class="border-b text-right">Rp. <?php echo number_format($tampil['jumlah'],0) ?></td>
                                <td class="border-b text-center"><?php echo $tampil['tgl']?></td>
                                <td class="border-b"><?php echo $tampil['bank']?></td>
                                <td class="border-b"><?php echo $tampil['nmrek']?></td>
                                <td class="border-b text-center"><?php echo $tampil['mode']?></td>
                                <td class="border-b text-center">
                                    <div class="flex sm:justify-center items-center">
                                        <a class="flex items-center mr-3" href="donasi_pemberi_rst_edit.php?kd=<?php echo $tampil['id']?>"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
                                        <a class="flex items-center text-theme-6" href="donasi_pemberi_rst_del.php?kd=<?php echo $tampil['id']?>" onclick="return confirm('Data akan dihapus. Lanjutkan?')"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete </a>
				</td>
                            </tr>
