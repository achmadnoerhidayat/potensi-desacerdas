...
session_start();
	...
	if(isset($_POST['submit1'])) {
		...
		mysqli_query($koneksi,"INSERT INTO tbrab 
				(kode, tahun, bidang, bidang_sub, kegiatan, waktu, rincian) 
				VALUES 
				('$kd_kel','$txttahun','$txtbidang','$txtbidangsub','$txtkeg','$txtwaktu','$txtrincian')");
		
		$cari_kd=mysqli_query($koneksi,"SELECT id from tbrab order by id desc limit 1");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$id=$tm_cari['id'];
		
		$next = 'location:adm_rab_input_sub.php?id=';
		$awal = $next.$id;
		header($awal);
	}
?>
