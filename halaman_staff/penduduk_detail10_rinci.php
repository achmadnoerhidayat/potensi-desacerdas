...
if(isset($_POST['btn1'])){
	...
	$cari_kd=mysqli_query($koneksi,"select bulan FROM tbbulan WHERE id='$bln'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$nm_bulan=$tm_cari['bulan'];
	
} else {
	$id_dukuh=$_POST['cbodukuh'];
		$bln=$_POST['cbobln'];
	$thn=$_POST['cbothn'];
													$cari_kd=mysqli_query($koneksi,"select nm_dukuh FROM tbdukuh where id_dukuh='$id_dukuh'");
													$tm_cari=mysqli_fetch_array($cari_kd);
...
<center>
		<font size="3"><b>REKAPITULASI JUMLAH PENDUDUK <?php echo $nm_bulan; ?> TAHUN <?php echo $thn; ?></b></font>
		<br>
		<font size="2">DUKUH/DUSUN <?php echo $nm_dukuh; ?></font>
		<br>
...
<input type="hidden" name="cbodukuh" value="<?php echo $id_dukuh; ?>"/>
<input type="hidden" name="cbobln" value="<?php echo $bln; ?>"/>
<input type="hidden" name="cbothn" value="<?php echo $thn; ?>"/>
												<tr class="">
...
<table class="table"> 
											<thead> 
												<tr> 
													...
													<th class="border border-b-2 whitespace-no-wrap"><font size="2"><b>Keterangan</b></font></th>
											...
											<?php 
												...
												if(isset($_POST['tambah'])){	
													...
													$result = mysqli_query($koneksi,"SELECT * FROM tbpenduduk 
																					where kode='$kd_kel' and id_dukuh='$id_dukuh' 
																					and month(tgl_register)='$bln' and year(tgl_register)='$thn' 
																					and (nama like '%$txtcari%' or nik like '%$txtcari%' or kk like '%$txtcari%')");
													} else {
													$result = mysqli_query($koneksi,"SELECT * FROM tbpenduduk where kode='$kd_kel' and id_dukuh='$id_dukuh' and 
													month(tgl_register)='$bln' and year(tgl_register)='$thn'");
												}
												...
												if(isset($_POST['tambah'])){	
													...
													$query = mysqli_query($koneksi,"select *, DATE_FORMAT(tgl_lhr,'%d/%m/%Y') AS tanggal from tbpenduduk 
													where kode='$kd_kel' and id_dukuh='$id_dukuh' 
													and month(tgl_register)='$bln' and year(tgl_register)='$thn' LIMIT $mulai, $halaman")or die(mysql_error);
												}
												$no =$mulai+1;
											...
											?>
												<tbody>
													<tr>
														...
														<td class="border text-center"><font size="2"><?php echo $data['menetap']; ?></font></td>
														<td class="border">
															<form action="penduduk_detail01.php" method="post">			
																<input type="hidden" name="txtid" value="<?php echo $data['id']; ?>"/>															
																<input type="hidden" name="txtform" value="10"/>
														...
														<td class="border">
																<a href="penduduk_del.php?txtid=<?php echo $data['id']?>&txtform=10" 
