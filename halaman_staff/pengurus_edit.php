...
session_start();
	...
	$id=$_GET['kd'];
	$cari_kd=mysqli_query($koneksi,"SELECT id_rt, nama, id_jabatan, file_photo, file_name FROM tbpengurus_rt WHERE id='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$id_rt=$tm_cari['id_rt'];
	$nama=$tm_cari['nama'];
	...
	$cari_kd=mysqli_query($koneksi,"SELECT id_dukuh FROM tbrt WHERE id_rt='$id_rt'");
	$tm_cari=mysqli_fetch_array($cari_kd);
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Home</a> 
					...
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="" class="breadcrumb--active">Edit Pengurus RT</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
