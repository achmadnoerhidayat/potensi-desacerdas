...
session_start();
	...
	$cari_kd=mysqli_query($koneksi,"SELECT * FROM tbjenis_ternak where kode='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$nama=$tm_cari['nama'];
	$kelompok=$tm_cari['kelompok'];
?>
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 lg:col-span-12">
                        ...
                        <div class="intro-y box">
								...
								<form class="form-horizontal" action="jenis_ternak_edit_proses.php" method="post">
<input type="hidden" name="txtid" value="<?php echo $id; ?>"/>
                            <div class="p-5" id="input">
                                <div class="preview">
									<div class="mt-3">
										<label><b>Jenis Ternak</b></label>
                                        						<input type="text" class="input w-full border mt-2" 
											id="txtjenis" name="txtjenis" value="<?php echo $nama; ?>" >
                                    					</div>	

									<div class="mt-3">
										<label><b>Kelompok Ternak</b></label>
													<select class="input pr-12 w-full border col-span-4" id="cbojenis" name="cbojenis" required>
														<option value="">Kelompok Ternak</option>
												<?php
													$q = mysqli_query($koneksi,"select distinct(kelompok) as kel FROM tbjenis_ternak");
													while ($row1 = mysqli_fetch_array($q)){
														$k_id           = $row1['kel'];
														$k_opis         = $row1['kel'];
												?>
											<option value='<?php echo $k_id; ?>' <?php if ($k_id == $kelompok){ echo 'selected'; } ?>><?php echo $k_opis; ?></option>
												<?php
													}
												?>
													</select>
