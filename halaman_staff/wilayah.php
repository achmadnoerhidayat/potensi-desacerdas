...
session_start();
	...
	$cari_kd=mysqli_query($koneksi,"SELECT deskripsiw FROM tbprofil WHERE kode='$kd_kel'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	...
	if(isset($_POST['submit1'])) 
	{
		...
		$txtkd_kel = $_POST['txtkd_kel1'];							
		mysqli_query($koneksi,"UPDATE tbprofil SET deskripsiw='$txtdeskripsi' WHERE kode='$txtkd_kel'");

	$cari_kd=mysqli_query($koneksi,"SELECT deskripsiw FROM tbprofil WHERE kode='$txtkd_kel'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$deskripsiw=$tm_cari['deskripsiw'];
		...
		$sql_insert="INSERT INTO tbdukuh 
			(kode, nm_dukuh, jumlah_rt, luas, persentase) 
			VALUES 
			('$txtkd_kel','$txtnm','$txtrt','$txtluas','$txtpersen')";

		mysqli_query($koneksi,$sql_insert);
		echo"<script>window.alert('Data Dukuh/Dusun Berhasil Disimpan!');window.location=('wilayah.php');</script>";

		}
	else if(isset($_POST['submit2'])) 
     	{
		$txtdeskripsi = $_POST['txtdeskripsi'];
		$txtkd_kel = $_POST['txtkd_kel'];							
		mysqli_query($koneksi,"UPDATE tbprofil SET deskripsiw='$txtdeskripsi' WHERE kode='$txtkd_kel'");
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Home</a> 
					...
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="" class="breadcrumb--active">Wilayah</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
                ...
                <!-- END: Top Bar -->
								...
								<form class="form-horizontal" enctype="multipart/form-data" action="wilayah.php" method="post">
<input type="hidden" name="txtkd_kel" value="<?php echo $kd_kel; ?>"/>
											...
											<textarea class="input w-full border mt-2" id="txtdeskripsi" name="txtdeskripsi" rows="3" placeholder="Deskripsi" ><?php echo $deskripsiw; ?></textarea>
						                                    </div>
						                                </div>
						                            </div>
						                        </div>  
											...
											<div class="mt-3">
                                        							<input type="text" class="input w-full border mt-2" id="txtnm" name="txtnm" autocomplete="off" placeholder="Isikan Nama Dukuh/Dusun">
                                        							<input type="text" class="input w-full border mt-2" id="txtrt" name="txtrt" autocomplete="off" placeholder="Jumlah RT">
                                        							<input type="text" class="input w-full border mt-2" id="txtluas" name="txtluas" autocomplete="off" placeholder="Luas Wilayah (ha)">
                                        							<input type="text" class="input w-full border mt-2" id="txtpersen" name="txtpersen" autocomplete="off" placeholder="Persentase %">
											</div>
											<div class="mt-3">
												<div class="relative mt-2 text-right">	
													<input type="hidden" name="txtkd_kel1" value="<?php echo $kd_kel; ?>"/>														
													<button type="submit" name="submit1" class="button text-white bg-theme-1 shadow-md mr-2"> Tambah Data Dukuh/Dusun </button>																			
												</div>
						                                        </div>
