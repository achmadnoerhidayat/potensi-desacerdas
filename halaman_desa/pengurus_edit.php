...
session_start();
	...
	$id=$_GET['kd'];
	$cari_kd=mysqli_query($koneksi,"SELECT id_rt, nama, id_jabatan FROM tbpengurus_rt WHERE id='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$id_rt=$tm_cari['id_rt'];
	$nama=$tm_cari['nama'];
	$id_jabatan=$tm_cari['id_jabatan'];
	
	$cari_kd=mysqli_query($koneksi,"SELECT kd_dukuh FROM tbrt WHERE id_rt='$id_rt'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$id_dukuh=$tm_cari['kd_dukuh'];
?>
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
					...
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="pengurus_rt.php" class="">Pengurus RT</a>
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
                ...
                <!-- END: Top Bar -->
								...
								<form class="form-horizontal" enctype="multipart/form-data" action="pengurus_edit_proses.php" method="post">
						                            ...
						                            <div class="p-5">
						                                <div class="grid grid-cols-12 gap-5">
						                                    <div class="col-span-12 xl:col-span-12">
																<div class="mt-3">
																	<label><b>Pilih Dukuh/Dusun</b></label>
																...
																<div class="mt-3">
																	<label><b>Nama Pengurus RT</b></label>
																	<input type="text" class="input w-full border mt-2" 
																	id="txtnm" name="txtnm" value="<?php echo $nama; ?>" autocomplete="off" required placeholder="Nama Pengurus RT">
						                                        </div>
																																<div class="mt-3">
																	<div>
																		<label><b>Upload Foto Pengurus RT</b>&nbsp;
																...
																<div class="mt-3">
																	...
																	<?php
																		$q = mysqli_query($koneksi,"select kode, jabatan FROM tbjabatan_pengurus");
																		while ($row1 = mysqli_fetch_array($q)){
																			$k_id           = $row1['kode'];
																			$k_opis         = $row1['jabatan'];
																	?>
